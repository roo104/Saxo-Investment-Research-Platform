...
function entry(over: Partial<PortfolioEntry>): PortfolioEntry {
    return {
        ...
        quantity: 1, openingPrice: 100, bid: null, ask: null, mid: null, currency: 'USD',
        marketState: null, delayedByMinutes: null, priceAvailable: false, lastClose: null,
        exchange: null, country: 'United States', marketOpen: null, ...over,
    }
}

const entries: PortfolioEntry[] = [
    entry({id: 1, assetType: 'Stock', country: 'United States', currency: 'USD', quantity: 2, mid: 100}),
    entry({id: 2, assetType: 'Stock', country: 'Denmark', currency: 'DKK', quantity: 1, mid: 100}),
    entry({id: 3, assetType: 'FxSpot', country: null, currency: 'USD', quantity: 100, mid: 1}),
]

describe('AllocationOverview', () => {
    it('renders the three breakdown cards', () => {
    ...
    it('weights the Assets legend by market value', () => {
        ...
        const card = screen.getByRole('heading', {name: 'Assets'}).closest('.alloc-card')!
    ...
    it('draws a donut segment per slice with an accessible summary', () => {
        render(<AllocationOverview entries={entries}/>)
        const donut = screen.getByRole('heading', {name: 'Country'}).closest('.alloc-card')!
